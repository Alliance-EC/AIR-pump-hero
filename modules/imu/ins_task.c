...
// static float last_pitch_angle    = 0; // 上一次的pitch角度
// static float last_yaw_angle_total;
...
INS_Instance *INS_Init(BMI088Instance *bmi088)
{
    ...
    for (uint8_t i = 0; i < 3; i++)
    {
        ...
        accel_offset[i] = bmi088->accel_offset[i];
    }
...
void INS_Task(void)
{
    ...
    //  if (INS->output.INS_angle[INS_YAW_ADDRESS_OFFSET] + yaw_round_count * 2 * PI - INS->output.Yaw_total_angle >= 0.01 || INS->output.INS_angle[INS_YAW_ADDRESS_OFFSET] + yaw_round_count * 2 * PI - INS->output.Yaw_total_angle < 0.01)
    ...
    // 陀螺仪漂移问题无法解决
    // if (INS->output.Yaw_total_angle - last_yaw_angle_total <= ERROR_IN_ANGLE && INS->output.Yaw_total_angle - last_yaw_angle_total >= -ERROR_IN_ANGLE)
    //     INS->output.Yaw_total_angle = last_yaw_angle_total;
    // if (INS->output.INS_angle[2] - last_pitch_angle <= ERROR_IN_ANGLE && INS->output.INS_angle[2] - last_pitch_angle >= -ERROR_IN_ANGLE)
    //     INS->output.INS_angle[2] = last_pitch_angle;
    // last_pitch_angle     = INS->output.INS_angle[2];
    // last_yaw_angle_total = INS->output.Yaw_total_angle;
